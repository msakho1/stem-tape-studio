...
import { initialSurfaceState, pressControl, reduceSurface, releaseControl, type SurfaceState } from "@/machine/surface";
...
function press(s: SurfaceState, ...controls: Control[]) {
  ...
  if (controls.length >= 2) next = reduceSurface(next, { type: "chordStart", controls, t: 0 });
  return next;
}
...
describe("Track-button chord audition", () => {
  ...
  it("does not mute lanes when the chord releases", () => {
    ...
    s = reduceSurface(s, { type: "chordRelease", controls: ["track-button-1", "track-button-2"], releaseSpreadMs: 20, t: 10 });
    expect(last(s, "lane.audition")?.payload["mask"]).toBe("");
    // the deferred taps that follow are consumed by the chord, not muted
    s = reduceSurface(s, { type: "tap", control: "track-button-1", count: 1, t: 20 });
    s = reduceSurface(s, { type: "tap", control: "track-button-2", count: 1, t: 21 });
  ...
  it("narrows the chord when one member lifts", () => {
    ...
    s = reduceSurface(s, { type: "holdEnd", control: "track-button-2", level: "hold", duration: 600, t: 30 });
  ...
  it("leaves a single Track press as a normal mute", () => {
    ...
    s = reduceSurface(s, { type: "tap", control: "track-button-1", count: 1, t: 5 });
