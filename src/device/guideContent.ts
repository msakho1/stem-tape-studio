...
const PERFORMANCE: [string, string][] = [
  ...
  ["heads.source", "Heads always read the Vocal stem"],
  ["heads.mute", "Tap a Track to mute or unmute its head"],
  ["heads.placement", "The four heads start a quarter of the cycle apart"],
  ["heads.entry.safe", "Head 1 alone is audible on entry"],
  ["heads.focus", "The head you last touched stays in front"],
  ["heads.reverse", "Double-tap a Track to reverse that head"],
...
“MIDI works in supported browsers such as desktop Chrome and Edge. It is not currently available in iPhone or iPad browsers.”`,
  ...
  },
  {
    ...
    body:
      "Hold FUNCTION and triple-tap PLAY. Heads always uses the Vocal stem. The four heads start a quarter of the cycle apart: head 1 sits exactly where the vocal is playing, the others a quarter, a half and three quarters further on. Head 1 is the only one you hear when you enter — the ordinary vocal is replaced, not stacked underneath, so entering sounds the same as before. Add heads 2–4 deliberately: tap a Track to mute or unmute its head. The head you last unmuted, moved or scrubbed stays in the foreground and the others sit behind it as supporting layers. Faders set head levels, FUNCTION + fader scrubs a head, double-tap a Track reverses it without muting it, and PRINT still bakes what you build to an empty track. Drums, bass and instruments carry on untouched, and leaving Heads returns the vocal at the song’s current position with the mix exactly as it was.",
    highlight: ["function", "play"],
    motion: "triple",
    held: ["function"],
    features: [
