...
describe("first-claim ownership of PLAY", () => {
  ...
  it("Track at 449 ms claims PLAY and cancels the global-loop hold", () => {
    const { a } = harness();
    a.handle({ phase: "down", control: "play", t: 0, id: "play", pointerId: 1 });
    expect(a.isClaimed("play")).toBe(false);
    a.handle({ phase: "down", control: "track-button-1", t: CLAIM - 1, id: "track-button-1", pointerId: 1 });
  ...
  it("Track at exactly 450 ms does NOT claim: the global loop owns PLAY", () => {
    ...
    a.handle({ phase: "down", control: "play", t: 0, id: "play", pointerId: 1 });
    a.handle({ phase: "down", control: "track-button-1", t: CLAIM, id: "track-button-1", pointerId: 1 });
    expect(a.isClaimed("play")).toBe(false);
    expect(a.isClaimed("track-button-1")).toBe(false);
    a.handle({ phase: "up", control: "track-button-1", t: CLAIM + 100, id: "track-button-1", pointerId: 1 });
  ...
  it("Track well after the hold (2 s) still behaves normally", () => {
    const { a, intents } = harness();
    a.handle({ phase: "down", control: "play", t: 0, id: "play", pointerId: 1 });
    a.handle({ phase: "down", control: "track-button-3", t: 2000, id: "track-button-3", pointerId: 1 });
    a.handle({ phase: "up", control: "track-button-3", t: 2100, id: "track-button-3", pointerId: 1 });
...
describe("solo / link boundary at 700 ms", () => {
  it("release at 699 ms overlap latches solo", () => {
    const { a, intents } = harness();
    a.handle({ phase: "down", control: "play", t: 0, id: "play", pointerId: 1 });
    a.handle({ phase: "down", control: "track-button-2", t: 100, id: "track-button-2", pointerId: 1 });
    vi.advanceTimersByTime(LINK - 1);
    a.handle({ phase: "up", control: "track-button-2", t: 100 + LINK - 1, id: "track-button-2", pointerId: 1 });
  ...
  it("reaching 700 ms while held fires link, and the later release is inert", () => {
    const { a, intents } = harness();
    a.handle({ phase: "down", control: "play", t: 0, id: "play", pointerId: 1 });
    a.handle({ phase: "down", control: "track-button-4", t: 100, id: "track-button-4", pointerId: 1 });
    vi.advanceTimersByTime(LINK);
    expect(intents).toHaveLength(1);
    expect(intents[0]).toMatchObject({ type: "stem.link", stem: 3, overlapMs: LINK });
    a.handle({ phase: "up", control: "track-button-4", t: 100 + LINK + 300, id: "track-button-4", pointerId: 1 });
  ...
  it("PLAY released first before 700 ms still latches solo at the real overlap", () => {
    const { a, intents } = harness();
    a.handle({ phase: "down", control: "play", t: 0, id: "play", pointerId: 1 });
    a.handle({ phase: "down", control: "track-button-1", t: 200, id: "track-button-1", pointerId: 1 });
    vi.advanceTimersByTime(300);
    a.handle({ phase: "up", control: "play", t: 500, id: "play", pointerId: 1 });
  ...
  it("a lost pointer cancels the pending chord without emitting anything", () => {
    const { a, intents } = harness();
    a.handle({ phase: "down", control: "play", t: 0, id: "play", pointerId: 1 });
    a.handle({ phase: "down", control: "track-button-1", t: 100, id: "track-button-1", pointerId: 1 });
    a.handle({ phase: "cancel", control: "track-button-1", t: 300, id: "track-button-1", pointerId: 1 });
    vi.advanceTimersByTime(2000);
    expect(intents).toEqual([]);
  });
  ...
  it("both stems can be soloed in sequence — each Track arms its own decision", () => {
    const { a, intents } = harness();
    a.handle({ phase: "down", control: "play", t: 0, id: "play", pointerId: 1 });
    a.handle({ phase: "down", control: "track-button-1", t: 50, id: "track-button-1", pointerId: 1 });
    a.handle({ phase: "down", control: "track-button-2", t: 80, id: "track-button-2", pointerId: 1 });
    vi.advanceTimersByTime(200);
    a.handle({ phase: "up", control: "track-button-1", t: 250, id: "track-button-1", pointerId: 1 });
    a.handle({ phase: "up", control: "track-button-2", t: 280, id: "track-button-2", pointerId: 1 });
