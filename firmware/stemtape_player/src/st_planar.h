 ...
 *
 * THERE IS NO OTHER ARM, AND THAT IS A DECISION.
 *
 * A width-generic byte-assembling fallback briefly lived here so a historical
 * conformance harness could compile this header at 24 bits. It was removed:
 * v1.3 is deliberately a breaking storage migration that fails closed on v1.1
 * and v1.2 media, so a 24-bit decoder is a path the shipped firmware can never
 * take -- and the place it sat is the innermost loop of the 48 kHz render.
 * Dead code there is not free even when it compiles to nothing: it is the one
 * function where a future reader most needs to see exactly one way for a
 * sample to be read. The gate that wanted it was rescoped instead; see
