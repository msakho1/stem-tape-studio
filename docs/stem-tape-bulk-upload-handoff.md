...
### 6.1 Second physical attempt — RX ring desync after a real timeout, fixed

With the 64000ms/watchdog-feed fix in place, the second physical attempt
got further: attempt 0 genuinely ran the full ~64 seconds before the
device answered with a real `ERR_TIMEOUT_PAYLOAD` (status 3) — the new
ceiling is taking effect. The companion's automatic retry then received
an **undefined status value (76)** — not one of the 17 frozen codes in
§4, and not something firmware ever legitimately sends.

Root cause: `xfer_bulk_write_sector()`'s `ERR_TIMEOUT_PAYLOAD` path built
and sent its 14-byte response immediately on timeout, without draining
whatever host-to-device bytes were still sitting in the receive ring (a
partial fragment of the abandoned payload, or bytes still landing right
after the device gave up). On the host's retry, its fresh 17-byte request
header was read starting mid-stream instead of from a real header
boundary — corrupting `seq`/`dest_block`/`payload_len` with garbage and
producing a nonsense response the companion couldn't parse as any real
status. Fixed by draining the ring the moment the timeout is known, reusing
the same drain-to-clean-boundary idiom already used elsewhere in this
firmware for exactly this class of problem, adapted only in that this path
still owes the host a real structured response (not a raw resync byte).
This is a one-shot immediate drain, not a wait for more bytes — it cannot
fully protect against a host that is still genuinely transmitting well
after the device's 64s ceiling gives up; that is a transport-level
condition, not something a bounded device-side drain can guarantee against.

**Still open, and the more consequential question:** the second attempt's
own timing shows attempt 0 took the *entire* 64 seconds to fail — 8192
bytes did not arrive in that window at all, implying real throughput far
below the ~712ms theoretical line rate for 115200 8N1. Raising the
firmware timeout further is very unlikely to be the fix for that; it
points at the companion/OS/browser transmission path, not device-side
sizing. The `writeMs`/`ackMs` split added in §6 is now captured durably
(survives a mid-session reconnect — see the companion-side transport.ts
change) specifically so the *next* physical attempt's data can show
whether the delay is host-side (`writeMs`) or device-side (`ackMs`)
...
- **Firmware commit:** `812b27f62958d6655ea95516cbdf1b1cbb6cb217` (branch
  `claude/stemtape-m0-safety-audit-1vg9pq`; a content-free CI-retrigger
  commit on top of the real fix at `79724a8f0f8ba0562cbf95650b3e7a724765c700`,
  which is what actually changed) — CI-verified green: the real
  `stemtape-player` job (workflow run
  [32305064107](https://github.com/msakho1/stem-tape-studio/actions/runs/32305064107),
  job [96235930712](https://github.com/msakho1/stem-tape-studio/actions/runs/32305064107/job/96235930712))
  completed successfully, including the runtime symbol-presence gate and
  the strict persistence safety gate (`GATE PASSED — no persistent-write
  capability outside the 2 proven, session-bounded eMMC adapter
  function(s) (xfer_bulk_write_sector() and xfer_v11_write())`). This
  commit contains everything in Slices C1–C4 (wire contract, production
  dispatcher wiring, post-commit runtime reload, `'Y'` benchmark removal)
  plus two fixes driven by real physical-test diagnostics: C5
  (`ST_BULK_PAYLOAD_TIMEOUT_MS` scaled to the payload's real size, plus
  the `cdc_rx()` watchdog-feed fix that made raising it safe) and this
  slice's RX ring-drain fix (§6 below) for the desync a genuine
  `ERR_TIMEOUT_PAYLOAD` could leave behind on the host's next retry.
- **BIN artifact** (from that same real, green CI run — every value
  ...
  | file | SHA-256 | size |
  | --- | --- | --- |
  | `stemtape_player.bin` | `b791e8433f1506cf281a305cab46c1b69498b16c268677eebb5534cf9e6244a1` | 98,972 bytes |
  | `zephyr.bin` | `b791e8433f1506cf281a305cab46c1b69498b16c268677eebb5534cf9e6244a1` | 98,972 bytes (identical to `stemtape_player.bin`) |
  | `zephyr.elf` | `26e17776f35800bb55afbf13c1d641fed8c5cc88656e1868fb839c992515a340` | — |
  | `zephyr.hex` | `9d9eb1574d8bdc29997359df378cfc39186daaae1a2a2142f65a5f380ce445fe` | — |

  Downloadable from the `stemtape-player-audit` GitHub Actions artifact:
  <https://github.com/msakho1/stem-tape-studio/actions/runs/32305064107/artifacts/9384475947>
