  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "SP-1 Uploader — Stem Tape" },
      {
        ...
        content:
          "Connect the SP-1 over USB, prepare four stereo 24-bit stems locally, and inspect the song library.",
      },
...
const fmtBytes = (n: number) =>
  n < 1024 * 1024 ? `${(n / 1024).toFixed(0)} KiB` : `${(n / 1048576).toFixed(1)} MiB`;
...
function DevicePage() {
  ...
  const [description, setDescription] = useState<ReturnType<StemTapeTransport["describe"]> | null>(
    null,
  );
  ...
  const refresh = useCallback(async () => {
    ...
    if (bad)
      say(
        "This SP-1's song index is uninitialised or written by different firmware. Nothing was changed.",
      );
  }, [say]);
  ...
  const setStem = useCallback(
    async (name: StemSlotName, file: File) => {
      ...
      try {
        ...
        say(
          `${STEM_LABEL[name]}: ${file.name} · ${rate} · ${buf.numberOfChannels} ch · ${fmtSecs(buf.duration)}`,
        );
      } catch (e) {
        say(
          `${STEM_LABEL[name]}: could not decode ${file.name} — ${e instanceof Error ? e.message : String(e)}`,
        );
      } finally {
        void ac.close();
      }
  ...
  const prepare = useCallback(async () => {
    ...
    if (!metadataComplete) {
      say(
        "BPM and beat zero are required: Gate, Delay and loops are tempo-locked, so they are not optional notes.",
      );
      return;
    }
    setBusy(true);
  ...
  const initialiseIndex = useCallback(async () => {
    ...
    if (!t) return;
    if (
      !confirm("Write a fresh, empty song index to this SP-1? Existing songs become unreachable.")
    )
      return;
    setBusy(true);
    try {
  ...
  const removeSlot = useCallback(
    async (index: number) => {
      const t = transportRef.current;
      if (!t) return;
      if (!confirm(`Delete song ${index + 1} from the SP-1? Its four tracks become unreachable.`))
        return;
      setBusy(true);
      try {
    ...
    <div className="min-h-screen">
          ...
          <Link to="/" className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-mono text-[11px] text-[var(--ink-dim)]">
                connect · four stems · prepare · inspect
              </p>
            </div>
          </Link>
        </div>
      ...
      <main className="mx-auto w-full max-w-[860px] px-4 pb-24 pt-6 md:px-8">
        <nav className="st-section" aria-label="Uploader stages" data-testid="stages">
          ...
          <p className="mt-2 font-mono text-[12px] text-[var(--ink-dim)]" data-testid="next-step">
            {nextStep}
          </p>
          <div className="mt-2 flex flex-wrap gap-3 font-mono text-[11px]">
            {mockMode && (
              <span
                className="border border-[var(--bench-line)] px-2 py-[2px] text-[var(--ink)]"
        ...
        <section className="st-section" data-testid="write-lock">
          <p className="st-section__title">
            {verdict.writable ? "compatibility negotiated" : "physical uploads locked"}
          </p>
        ...
        {supported === false && (
          <section className="st-section" data-testid="unsupported">
            ...
            <p className="font-mono text-[13px] leading-relaxed text-[var(--ink-dim)]">
              Direct USB transfer needs Web Serial: Chrome or Edge on desktop, or a supported
              Chromium browser on Android. iPhone and iPad Safari do not provide Web Serial — there
              is no way to transfer songs from iOS, and any page claiming otherwise is wrong.
            </p>
          </section>
        )}
        ...
        <section className="st-section">
          ...
          {connected && description && (
            <dl className="mt-3 grid grid-cols-2 gap-x-6 gap-y-1 font-mono text-[12px] text-[var(--ink-dim)] md:grid-cols-3">
              <div>
                device · <span className="text-[var(--ink)]">{description.deviceName}</span>
              </div>
              <div>
                transport · <span className="text-[var(--ink)]">{description.transport}</span>
              </div>
              <div>
                audio · <span className="text-[var(--ink)]">{description.audioFormat}</span>
              </div>
              <div>
                slots reported ·{" "}
                <span className="text-[var(--ink)]" data-testid="slots">
                  {description.slots}
                </span>
              </div>
              <div>
                library base ·{" "}
                <span className="text-[var(--ink)]">block {description.libraryBase}</span>
              </div>
              <div>
                per song · <span className="text-[var(--ink)]">{description.sectorsPerSong}</span>{" "}
                sectors ({fmtBytes(description.capacityBytesPerSong)})
              </div>
              <div>
                index blocks · <span className="text-[var(--ink)]">{description.indexBlocks}</span>
              </div>
              <div>
                generation · <span className="text-[var(--ink)]">{description.generation}</span>
              </div>
              <div>
                staging ·{" "}
                <span className="text-[var(--ink)]">{description.staging ? "yes" : "no"}</span>
              </div>
            </dl>
          )}
        </section>
        ...
        {connected && uninitialised && (
          <section className="st-section" data-testid="uninitialised">
            ...
            <p className="font-mono text-[13px] leading-relaxed text-[var(--ink-dim)]">
              Neither index slot holds a valid STIX v2 record, so no song list can be read. This is
              blank or corrupt storage — an interrupted upload never lands here, because the
              previous generation always survives.
            </p>
            <button
                ...
                >
                  ...
                  <span className="ml-auto flex gap-2">
                    <button
                      className="st-btn"
                      disabled={busy}
                      onClick={() => void verifySlot(s.index)}
                    >
                      Verify
                    </button>
                    <button
        ...
        <section className="st-section">
            ...
            {stemRows.map(({ name, file, buf, out }) => (
              ...
              >
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  />
                  <button className="st-btn" disabled={!file} onClick={() => preview(name)}>
                    Preview
                  </button>
                </div>
                {buf && (
                  <p className="text-[var(--ink-dim)]" data-testid={`info-${name}`}>
                    {file?.name} · {sourceRates[name] ?? buf.sampleRate} Hz · {buf.numberOfChannels}{" "}
                    ch · {fmtSecs(buf.duration)}
                    {out
                      ? ` · out ${out.pcm24.length} B stereo 24-bit · peak ${(out.peak * 100).toFixed(0)}%`
                      : ""}
          ...
          <div className="mt-3 grid gap-2 font-mono text-[12px] md:grid-cols-2">
            <label className="flex items-center gap-2">
              ...
              <input
                className="st-input flex-1 border border-[var(--bench-line)] bg-transparent px-2 py-1"
                value={title}
            ...
            <label className="flex items-center gap-2">
              ...
              <input
                className="st-input w-24 border border-[var(--bench-line)] bg-transparent px-2 py-1"
                data-testid="bpm"
                value={bpm}
                onChange={(e) => setBpm(e.target.value)}
              />
              <button className="st-btn" onClick={tap} data-testid="tap">
                Tap tempo
              </button>
            </label>
            <label className="flex items-center gap-2">
          ...
          <p className="mt-2 font-mono text-[11px] text-[var(--ink-faint)]">
            BPM and beat zero are required, not optional notes: Gate, Delay, loops and every other
            synchronised behaviour are tempo-locked. A song is never reported ready if the
            negotiated firmware cannot store them.
          </p>
        ...
        {song && (
          <section className="st-section" data-testid="review">
            ...
            <p
              className="font-mono text-[13px] leading-relaxed text-[var(--ink-dim)]"
              data-testid="prepared"
            >
              {song.metadata.title || "untitled"} — {fmtSecs(song.durationSeconds)},{" "}
              {song.metadata.bpm} BPM, beat zero at {song.metadata.downbeatSeconds}s. Four stems, 48
              kHz stereo 24-bit.
            </p>
            ...
            {capacity.note && (
              <p
                className="mt-1 font-mono text-[12px] text-[var(--ink-faint)]"
            ...
            {showTech && (
              <dl
                className="mt-2 grid gap-1 font-mono text-[11px] text-[var(--ink-faint)]"
                data-testid="tech"
              >
                <div>
                  frames · {song.frames} @ {SAMPLE_RATE} Hz
                </div>
                <div>audio bytes · {song.audioBytes}</div>
                <div>
                  sector bytes · {SECTOR_BYTES} ({BLOCKS_PER_SECTOR} × {PHYSICAL_BLOCK_BYTES} B
                  blocks)
                </div>
          ...
          <section className="st-section" data-testid="verify">
            ...
            <ul className="mt-3 grid gap-1 font-mono text-[12px]" data-testid="verification">
              ...
              <li data-testid="v-readback">
                {result.verification.deviceReadbackVerification ? "ok  " : "no  "}device readback
                verification (committed bytes re-read from the SP-1)
              </li>
              <li data-testid="v-playback">
                {playbackConfirmed && result.verification.deviceReadbackVerification
                  ? "ok  "
                  : "no  "}
                physical playback verification (you heard it play on the device)
              </li>
            </ul>
            <div className="mt-3 flex flex-wrap items-center gap-3">
              {result.outcome === "unknown" && (
        ...
        <section className="st-section">
          ...
          <pre
            className="max-h-[220px] overflow-auto whitespace-pre-wrap font-mono text-[11px] text-[var(--ink-dim)]"
            data-testid="log"
          >
            {log.join("\n")}
          </pre>
