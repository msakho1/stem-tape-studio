...
describe("LED Stage 2 — stem activity envelopes", () => {
  ...
  it.each([0, 1, 2, 3])("meters all four stems while stem %i is selected", (activeStem) => {
    const state = { ...loadedPlaying(), activeStem };
    // silent / loud / medium / silent
    ...
    expect(track.every((l) => !/active stem/.test(l.owner))).toBe(true);
    expect(track[1]!.brightness).toBeGreaterThan(track[2]!.brightness);
    expect(track[2]!.brightness).toBeGreaterThan(track[0]!.brightness);
    expect(track[0]!.brightness).toBe(track[3]!.brightness);
  });
  ...
  it("returns loaded stems to the stopped state when the transport stops", () => {
    ...
    // Stopped: selection may still breathe; loaded stems no longer meter.
    expect(frame.leds.slice(1, 4).map((l) => l.mode)).toEqual(["dim", "dim", "dim"]);
    expect(frame.leds[0]!.mode).not.toBe("activity");
  });
});
