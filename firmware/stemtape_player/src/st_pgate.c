...
{
	const st_pgate_win_t zero = { 0u, 0u, 0u, 0u };
	...
	g->test        = zero;
	g->was_playing = false;
	enter(g, ST_PGATE_SETTLE, now_ms, 0u, 0u);
}
			        ...
			        uint32_t worst_us)
{
	 ...
	 */
	if (!playing) {
		enter(g, ST_PGATE_SETTLE, now_ms, sectors, underruns);
		g->was_playing = false;
		return g->phase;
	}
	if (!g->was_playing) {
	...
	case ST_PGATE_SETTLE:
		if (now_ms - g->phase_ms >= ST_PGATE_SETTLE_MS) {
			enter(g, ST_PGATE_BASELINE, now_ms, sectors, underruns);
		}
		break;

	case ST_PGATE_BASELINE:
		if (now_ms - g->phase_ms >= ST_PGATE_WINDOW_MS) {
			close_window(g, &g->base, now_ms, sectors, underruns);
			enter(g, ST_PGATE_TEST, now_ms, sectors, underruns);
		}
		break;

	case ST_PGATE_TEST:
		if (now_ms - g->phase_ms >= ST_PGATE_WINDOW_MS) {
			close_window(g, &g->test, now_ms, sectors, underruns);
			/*
			 ...
			 */
			if (g->base.underruns > 0u) {
				g->verdict = ST_PGATE_VERDICT_INCONCLUSIVE;
			} else if (g->test.underruns > 0u) {
