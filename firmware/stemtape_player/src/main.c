...
/*
 * STEM TAPE Phase T0 -- throughput budget and physical benchmark.
 *
 * PURPOSE: measure real physical CDC/eMMC throughput at multiple transfer
 * granularities, so the T1-T5 fast-transfer architecture is built against
 * MEASURED numbers, not assumed ones -- see docs/stem-tape-benchmark-t0.md
 * for the full protocol spec, the report table, and how to run this on a
 * physical SP-1.
 *
 * SAFETY (the entire reason this can run against a real device today,
 * before any of T1-T5 exist): every write this issues goes through
 * xfer_v11_refresh_session() (the SAME function 'Q' already calls to
 * open/refresh g_v11_session fresh from the real on-disk index) and
 * st_ab_session_check_write() (the SAME per-block gate xfer_v11_write()
 * itself is bounded by) -- no parallel, unverified bounds-check exists
 * here. The benchmark never writes to an index region (only ever computes
 * a target inside the CURRENT inactive SONG region's own address range)
 * and never constructs, let alone writes, an ST11_INDEX_MAGIC record, so
 * st_ab_session_check_write()'s own magic-detection path is never even
 * reached -- there is no code path in this function that could ever
 * commit a new generation. Interrupting a benchmark run (power loss, host
 * disconnect, reset) leaves at most harmless pattern bytes inside the
 * CURRENTLY inactive song region, which a real upload would overwrite in
 * full anyway before ever computing its own checksums against it -- the
 * active song, active index, and both records' magic bytes are never
 * touched by any path below.
 *
 * RAM: deliberately adds ZERO new large static buffers. Every mode reuses
 * s_v11_verify_scratch (ST11_SECTOR_BYTES = 8192 B), already allocated
 * for the real commit path's own verify-before-commit step and otherwise
 * idle for the whole duration of a benchmark run (a benchmark never
 * reaches a magic-committing write, so the real verify step can never be
 * in flight at the same time -- see the safety paragraph above). This is
 * also why T0 does NOT test a single eMMC burst larger than one 8 KiB
 * sector (16 blocks): CONFIG_MAIN_STACK_SIZE is 4096 B (prj.conf) and the
 * measured free-RAM baseline at the start of this slice was ~41 KB (see
 * docs/stem-tape-per-stem-head-forward-feasibility.md's own measured
 * figure, adjusted for this session's own small Phase 3 additions) --
 * against the 32 KiB floor this standing constraint requires, that
 * leaves on the rough order of 8 KB of safe new-static-allocation
 * headroom, not comfortably enough to add a SECOND full scratch buffer
 * and still call the remaining margin a real floor rather than a number
 * chosen to just barely clear it. "Any larger transfer unit that fits
 * safely" is instead exercised as multiple chained 16-block (8 KiB)
 * CMD25 bursts within one benchmark call (BENCH_MODE_EMMC_WRITE/READ/
 * END_TO_END's own `total_blocks` can far exceed `per_op`) -- a real,
 * honestly-labeled measurement of whether a longer SESSION of same-size
 * bursts sustains higher throughput than issuing them one at a time from
 * the host, but NOT a claim about one single burst larger than 8 KiB.
 * docs/stem-tape-benchmark-t0.md states this limitation explicitly, with
 * the exact measured RAM headroom this commit's own real CI build
 * reports, so a true larger-single-burst test can be scoped with real
 * numbers once headroom is confirmed rather than guessed here.
 *
 * WIRE PROTOCOL ('Y', new -- P/Q/R/W/F/X are all unchanged):
 *   host -> device: 'Y' <mode:u8> <mode-specific params, LE>
 *   device -> host: <status:u8> ['y' only: 32-byte result record, LE>
 *   status: 'y' = ok (result follows), 'e' = rejected (params invalid,
 *     no v1.1 layout, no inactive region, or the operation itself
 *     failed) -- no result record follows 'e'.
 *   result record (8 x u32 LE = 32 bytes):
 *     [0] elapsed_ms            -- k_uptime_get() delta for this ONE call
 *     [1] bytes_or_units        -- meaning is mode-specific (bench_send_
 *                                  result()'s own doc comment)
 *     [2] cdc_rx_dropped_bytes  -- g_cdc_rx_dropped_bytes, CUMULATIVE
 *                                  since boot, not a delta for this call
 *                                  (the host diffs two results itself)
 ...
 *     [7] mode-specific -- 0 for every mode except CDC_ONLY, which
 *         reports the line's currently negotiated baud rate here (real
 *         hardware line-coding state via uart_line_ctrl_get(), 0 if the
 *         backend does not support the query) -- see bench_send_
 *         result()'s own doc comment
 *   Every cumulative counter is a REAL, already-existing diagnostic this
 *   codebase already maintains for the real data path (see sp1_emmc.c's
 *   own doc comment) -- not new instrumentation invented just for this
 *   benchmark, except g_cdc_rx_dropped_bytes (see cdc_rx_isr()'s own
 *   comment on why that ONE counter genuinely did not exist before).
 *
 * MODES:
 *   0 CDC_ONLY    params: total_bytes:u32. Drains total_bytes from the
 *                 CDC ring, touches no eMMC at all -- isolates host<->
 *                 device USB throughput from flash entirely. Capped at
 *                 4 MiB per call.
 *   1 EMMC_WRITE  params: per_op:u16 (1 or ST11_BLOCKS_PER_SECTOR only),
 *                 total_blocks:u32 (must be a multiple of per_op, capped
 *                 at BENCH_MAX_TOTAL_BLOCKS). Writes a fixed, deterministic,
 *                 never-all-zero, never-magic-shaped pattern -- generated
 *                 locally, never received over the wire -- to the current
 *                 inactive song region in bursts of per_op blocks each,
 *                 via the SAME emmc_write_blocks() the real path uses
 *                 (count=1 -> CMD24, count=16 -> CMD25). Pure eMMC-write
 *                 speed, no CDC/CRC cost included.
 *   2 EMMC_READ   Same params. Reads back total_blocks from the base the
 *                 most recent WRITE call used (or session-fresh base 0 if
 *                 none this boot). Pure eMMC-read speed.
 *   3 END_TO_END  Same params. The REAL path, timed as one number: CDC
 *                 receive of per_op*512 real bytes, st_crc32_compute()
 *                 over them (timed -- the real future validation cost,
 *                 not skipped), st_ab_session_check_write() per block,
 *                 emmc_write_blocks(). True host -> validate -> flash
 *                 throughput.
 *   4 FLUSH       No params. Times emmc_cache_flush() alone.
 *   5 READBACK    params: num_samples:u8 (2..16). Times a DETERMINISTIC
 *                 spread readback over the most recent WRITE call's own
 *                 range: sample 0 = first block, sample (n-1) = last
 *                 block, the rest evenly spaced between -- the exact
 *                 sampling SHAPE (first + last + even spread) T3's own
 *                 production readback rule will need real timing data
 *                 for.
 *
 * Every 'Y' call is a single, individually-bounded operation (never more
 * than BENCH_MAX_TOTAL_BLOCKS = 4096 blocks = 2 MiB of eMMC traffic, or
 * 4 MiB of pure CDC traffic, per call) so no single call can run long
 * enough to risk a k_uptime_get()-measured wraparound or starve the
 * watchdog main() feeds elsewhere in its own loop -- the host issues
 * repeated 'Y' calls back to back to build up a larger sample, exactly
 * like a real upload issues repeated 'W' calls today.
 */
#define BENCH_MODE_CDC_ONLY   0u
#define BENCH_MODE_EMMC_WRITE 1u
#define BENCH_MODE_EMMC_READ  2u
#define BENCH_MODE_END_TO_END 3u
#define BENCH_MODE_FLUSH      4u
#define BENCH_MODE_READBACK   5u

#define BENCH_MAX_TOTAL_BLOCKS (4096u)     /* 2 MiB of eMMC traffic per single 'Y' call */
#define BENCH_MAX_CDC_BYTES    (4u * 1024u * 1024u)

/* Most recent WRITE call's own target range, in the CURRENT inactive song
 * region -- READ/READBACK default to it so a "write then read back the
 * same range" pair is the natural, zero-extra-params call sequence. Reset
 * to 0/0 (meaning "not yet established this boot") at declaration; a READ/
 * READBACK issued before any WRITE this boot session_open()s fresh and
 * simply targets the region's own block 0, which is always a valid,
 * in-region address the moment a v1.1 layout exists. */
static uint32_t s_bench_last_base;
static uint32_t s_bench_last_count;

/* Not in sp1_emmc.h's own extern list (only controls_diag() locally
 * extern's it today) -- same symbol, declared here too so bench_send_
 * result() below can read it. */
extern volatile uint32_t emmc_dbg_cmd_retries;

static void bench_send_error(void)
{
	uint8_t e = 'e';
...
/* See this whole section's own doc comment for the record layout.
 * `field7` is 0 for every mode except CDC_ONLY, where its caller passes
 * the line's CURRENTLY NEGOTIATED baud rate (see xfer_bench_run()'s own
 * CDC_ONLY case) -- real hardware line-coding state, not the fixed
 * "reserved" 0 every other mode leaves it at, answering "does Web
 * Serial's configured baud value actually limit this USB CDC
 * implementation" with a real number alongside the real measured
 * throughput in the SAME result record, rather than requiring a second
 * round trip or an assumption either way. */
static void bench_send_result(uint32_t elapsed_ms, uint32_t bytes_or_units, uint32_t field7)
{
	uint32_t r[8];

	r[0] = elapsed_ms;
	r[1] = bytes_or_units;
	r[2] = (uint32_t)atomic_get(&g_cdc_rx_dropped_bytes);
	r[3] = emmc_dbg_cmd_retries;
	r[4] = emmc_dbg_busy_timeouts;
	r[5] = emmc_crc_wr_errs;
	r[6] = emmc_crc_rd_errs;
	r[7] = field7;

	uint8_t ok = 'y';

	cdc_tx(&ok, 1);
	cdc_tx((const uint8_t *)r, sizeof r);
}

/* Opens/refreshes g_v11_session (via the SAME real function 'Q' already
 * uses) and computes the current inactive song region's own base block
 * and block-capacity. Returns false (nothing written to *out_base/*
 * out_cap) if there is no usable v1.1 layout or the session failed to
 * open -- callers must bench_send_error() and return in that case, never
 * proceed with a stale or zeroed base. */
static bool bench_inactive_song_region(uint32_t *out_base, uint32_t *out_cap)
{
	if (!g_v11_layout_ready) {
		return false;
	}
	xfer_v11_refresh_session();
	if (!g_v11_session.open) {
		return false;
	}
	*out_base = (g_v11_session.inactive_song_slot == ST11_SLOT_A) ? g_v11_session.layout.song_a_start
									: g_v11_session.layout.song_b_start;
	/* Regions A and B are allocated at the SAME fixed capacity -- the
	 * real 'Q'/xfer_v11_refresh_session() path already relies on this
	 * exact assumption (it passes g_v11_layout.song_a_blocks as the
	 * capacity ceiling for EITHER slot's session, see that function's
	 * own call site) -- reused here, not invented for this benchmark. */
	*out_cap = g_v11_layout.song_a_blocks;
	return true;
}

/* Fills `buf` (ST11_SECTOR_BYTES) with a fixed, deterministic pattern that
 * is never all-zero and never begins with ST11_INDEX_MAGIC or an STSC
 * sector magic -- purely cosmetic here (this data is only ever written to
 * the SONG region, which st_ab_session_check_write()'s magic-detection
 * path never even inspects), but keeping it visibly "obviously benchmark
 * data, not a real recording" if anyone ever reads it back by hand costs
 * nothing. */
static void bench_fill_pattern(uint8_t *buf, uint32_t len)
{
	for (uint32_t i = 0; i < len; i++) {
		buf[i] = (uint8_t)(0xA5u ^ i ^ (i >> 8));
	}
}

/*
 * Returns -1 on every rejected/failed path (never reaching the real eMMC
 * write call past it), 0 on every accepted path -- the SAME `return -1;`
 * early-guard idiom xfer_v11_write() itself uses, deliberately, so this
 * function fits the strict persistence safety gate's own existing,
 * proven verification pattern (see .github/scripts/stemtape_player_
 * safety_gate.py's ALLOWED_WRITE_FUNCS entry for "xfer_bench_run") rather
 * than asking that gate to special-case a different shape. The caller
 * (xfer_service()) does not use the return value -- every path already
 * sends its own status byte over CDC before returning.
 */
static int xfer_bench_run(void)
{
	uint8_t mode;

	if (!cdc_rx(&mode, 1, 1000)) {
		xfer_resync('e');
		return -1;
	}

	switch (mode) {
	case BENCH_MODE_CDC_ONLY: {
		uint8_t p[4];

		if (!cdc_rx(p, sizeof p, 1000)) {
			xfer_resync('e');
			return -1;
		}
		uint32_t total = (uint32_t)p[0] | ((uint32_t)p[1] << 8) | ((uint32_t)p[2] << 16) |
				 ((uint32_t)p[3] << 24);

		if (total == 0u || total > BENCH_MAX_CDC_BYTES) {
			bench_send_error();
			return -1;
		}
		...
		while (got < total) {
			...
			if (!cdc_rx(chunk, want, 2000)) {
				break; /* stalled -- report what actually arrived, not an error */
			}
			got += want;
		}
		uint32_t elapsed = (uint32_t)(k_uptime_get() - t0);
		/* Real hardware line-coding state, not an assumption either way
		 * (see bench_send_result()'s own doc comment on `field7`) --
		 * uart_line_ctrl_get() returning nonzero (some backends don't
		 * support a baud-rate query on a virtual USB CDC ACM line at
		 * all) reports 0 here, which the host-side report already
		 * treats as "unknown/unsupported", never a fabricated value. */
		uint32_t baud = 0u;

		(void)uart_line_ctrl_get(cdc, UART_LINE_CTRL_BAUD_RATE, &baud);

		bench_send_result(elapsed, got, baud);
		break;
	}
	case BENCH_MODE_EMMC_WRITE:
	case BENCH_MODE_EMMC_READ:
	case BENCH_MODE_END_TO_END: {
		uint8_t p[6];
		...
		uint16_t per_op = (uint16_t)p[0] | ((uint16_t)p[1] << 8);
		uint32_t total_blocks = (uint32_t)p[2] | ((uint32_t)p[3] << 8) | ((uint32_t)p[4] << 16) |
					 ((uint32_t)p[5] << 24);

		if ((per_op != 1u && per_op != ST11_BLOCKS_PER_SECTOR) || total_blocks == 0u ||
		    total_blocks > BENCH_MAX_TOTAL_BLOCKS || (total_blocks % per_op) != 0u) {
			bench_send_error();
			return -1;
		}
		uint32_t base, cap;

		if (!bench_inactive_song_region(&base, &cap) || total_blocks > cap) {
			bench_send_error();
			return -1;
		}
		...
		if (mode == BENCH_MODE_EMMC_WRITE) {
			bench_fill_pattern(s_v11_verify_scratch, ST11_SECTOR_BYTES);
			for (uint32_t off = 0u; ok && off < total_blocks; off += per_op) {
				for (uint32_t k = 0u; k < per_op; k++) {
					if (st_ab_session_check_write(&g_v11_session, base + off + k,
								       s_v11_verify_scratch) != ST_AB_WRITE_OK) {
						ok = false;
						break;
					}
				}
				if (ok) {
					ok = emmc_write_blocks(base + off, s_v11_verify_scratch, per_op);
				}
			}
		} else if (mode == BENCH_MODE_EMMC_READ) {
			/* Prefers the most recent WRITE call's own base over the
			 * FRESH `base` just computed above: if the library changed
			 * between calls (a real upload landed in between two
			 * benchmark calls) this could target what is NOW the
			 * active region instead of the current inactive one --
			 * semantically loose for a "read back what I just wrote"
			 * pairing, but never unsafe: s_bench_last_base can only
			 * ever hold a value bench_inactive_song_region() itself
			 * once returned, so it is always a real, in-device,
			 * in-region v1.1 address -- and 'R' (above) already treats
			 * reading ANY v1.1 region, active or inactive, as
			 * unconditionally safe (see that handler's own comment). */
			uint32_t read_base = (s_bench_last_count != 0u) ? s_bench_last_base : base;

			for (uint32_t off = 0u; ok && off < total_blocks; off += per_op) {
				ok = emmc_read_blocks(read_base + off, s_v11_verify_scratch, per_op);
			}
		} else { /* BENCH_MODE_END_TO_END: the real path, timed as one number */
			for (uint32_t off = 0u; ok && off < total_blocks; off += per_op) {
				uint32_t nbytes = (uint32_t)per_op * ST11_PHYSICAL_BLOCK_BYTES;

				if (!cdc_rx(s_v11_verify_scratch, nbytes, 4000)) {
					ok = false;
					break;
				}
				(void)st_crc32_compute(s_v11_verify_scratch, nbytes); /* timed, real future cost */
				for (uint32_t k = 0u; k < per_op; k++) {
					if (st_ab_session_check_write(&g_v11_session, base + off + k,
								       s_v11_verify_scratch) != ST_AB_WRITE_OK) {
						ok = false;
		...
		if (mode != BENCH_MODE_EMMC_READ) {
			s_bench_last_base = base;
			s_bench_last_count = total_blocks;
		}
		bench_send_result(elapsed, total_blocks * ST11_PHYSICAL_BLOCK_BYTES, 0u);
		break;
	}
	case BENCH_MODE_FLUSH: {
		int64_t t0 = k_uptime_get();
		bool ok = emmc_cache_flush();
		uint32_t elapsed = (uint32_t)(k_uptime_get() - t0);
		...
		bench_send_result(elapsed, 0u, 0u);
		break;
	}
	case BENCH_MODE_READBACK: {
		uint8_t p[1];
		...
		if (n < 2u || n > 16u || s_bench_last_count == 0u) {
			bench_send_error();
			return -1;
		}
		bool ok = true;
		...
		for (uint32_t i = 0u; i < n && ok; i++) {
			uint32_t idx = (i == 0u)     ? 0u
				       : (i == n - 1u) ? s_bench_last_count - 1u
						       : (uint32_t)(((uint64_t)i * (s_bench_last_count - 1u)) /
								     (n - 1u));

			ok = emmc_read_blocks(s_bench_last_base + idx, s_v11_verify_scratch, 1u);
		}
		uint32_t elapsed = (uint32_t)(k_uptime_get() - t0);
		...
		bench_send_result(elapsed, n, 0u);
		break;
	}
	default:
		bench_send_error();
...
static void xfer_service(void)
{
	 ...
	 * comment (immediately above xfer_v11_write() -- STEM TAPE Phase T0
	 * section) for the full wire protocol, every mode, and the safety
	 * argument for why this can run against a real, live-song-carrying
	 * device today. */
	} else if (cmd == 'Y') {
		(void)xfer_bench_run();
	}
